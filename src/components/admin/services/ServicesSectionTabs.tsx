...
interface ServicesSectionTabsProps {
  tabs: Tab[]
  children: React.ReactNode[]
}

export function ServicesSectionTabs({ tabs, children }: ServicesSectionTabsProps) {
  ...
  return (
    <div className="space-y-0">
      ...
      <div className="border-b border-slate-100 bg-white rounded-t-xl overflow-x-auto">
        <nav className="flex min-w-max px-1 pt-1" aria-label="Services sections">
          {tabs.map((tab, i) => (
            <button
              key={tab.id}
              onClick={() => setActive(i)}
