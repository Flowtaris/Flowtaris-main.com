...
export function SocialLinksForm({ initialLinks }: { initialLinks: SocialLink[] }) {
  ...
  const handleAdd = async () => {
    ...
    setAdding(true)
    const newLink = { platform_name: newPlatform, url: newUrl, priority: 0 }
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        {links.sort((a, b) => b.priority - a.priority).map(link => (
          <div key={link.id} className="flex items-center gap-4 bg-slate-50 p-4 rounded-lg border border-slate-200">
      ...
      <div className="bg-white border border-slate-200 p-4 rounded-lg shadow-sm flex items-end gap-4">
        <div className="flex-1">
