...
const DEFAULT_TERMS = {
  ...
  sections: [
    {
      heading: '1. Acceptance of Terms & Master Services Agreement (MSA)',
      content: 'These Terms of Service govern the use of the Flowtaris AI platform and website. For enterprise customers, these terms operate in conjunction with your specific Master Services Agreement (MSA) and Service Level Agreement (SLA). In the event of a conflict between these terms and your executed MSA, the terms of the MSA shall prevail.'
    },
    {
      heading: '2. Enterprise Service Level Agreements (SLA)',
      content: 'Flowtaris AI guarantees a **99.99% uptime** for our core automation engine and API gateways. Service credits are automatically applied for any downtime exceeding this threshold, as detailed in your MSA. Scheduled maintenance windows will be communicated to your administrative contacts with a minimum of 48 hours\' notice.'
    },
    {
      heading: '3. Intellectual Property: You Own Your AI Outputs',
      content: 'Flowtaris AI retains all intellectual property rights to our proprietary automation platform, OCR engines, LLM orchestration layers, and software code. However, **you retain 100% ownership of all data inputted into the system and all outputs generated by the AI on your behalf.** Flowtaris AI claims no rights to your financial records, generated analyses, or conversational ERP interactions.'
    },
    {
      heading: '4. Acceptable Use & API Rate Limiting',
      content: 'Access to the Flowtaris AI platform and our ERP integration endpoints is subject to fair use and programmatic rate limiting to protect system stability. Customers agree not to:\n\n*   Reverse engineer, decompile, or attempt to extract the source code of our models.\n*   Use the platform to process data that violates international sanctions or anti-money laundering (AML) regulations.\n*   Attempt to bypass security controls, RBAC permissions, or tenant isolation mechanisms.'
    },
    {
    ...
    {
      heading: '7. Contact Legal',
      content: 'For MSA negotiations, SLA inquiries, or general legal questions, please contact our counsel:\n\n**Flowtaris Legal & Compliance**\nEmail: legal@flowtaris.com'
    }
  ]
}
...
export default function TermsConfigPage() {
  ...
  useEffect(() => {
    fetch('/api/ai/site-config')
      .then(res => res.json())
      .then(config => {
        if (config.terms_config && config.terms_config.sections) {
          setData(config.terms_config)
        } else if (config.terms_config && config.terms_config.content) {
          // Migration from markdown string to sections
          setData({
            ...config.terms_config,
            sections: [
              { heading: 'Terms of Service Content', content: config.terms_config.content }
            ]
          })
        }
        setLoading(false)
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/ai/site-config', {
        ...
        body: JSON.stringify({ terms_config: data })
      })
      if (!res.ok) throw new Error('Failed to save')
      setMessage({ type: 'success', text: 'Terms of Service configuration saved successfully.' })
      setHasChanges(false)
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Terms of Service Configuration</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            Manage the structured sections of your public Terms of Service page.
          </p>
        </div>
        <ViewLiveButton href="/terms" />
      </div>
        ...
        <Section title="Page Settings" description="Main title, badge, and effective date" icon={Shield}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <Field label="Page Title" hint="The main H1 title shown at the top">
              <TextInput value={data.title} onChange={v => updateField('title', v)} placeholder="Terms of Service" />
            </Field>
      ...
      <FloatingSaveBar 
        ...
        onDiscard={() => {
          ...
          fetch('/api/ai/site-config')
            .then(res => res.json())
            .then(config => {
              if (config.terms_config && config.terms_config.sections) setData(config.terms_config)
              else setData(DEFAULT_TERMS)
              setLoading(false)
              setHasChanges(false)
            })
        }}
