...
export default function EditCaseStudyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (!isNew) {
      fetch(`/api/ai/case-studies/${resolvedParams.id}`)
        .then(res => res.json())
        .then(json => {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = isNew ? '/api/ai/case-studies' : `/api/ai/case-studies/${resolvedParams.id}`
      ...
      router.push('/admin/ai/case-studies')
    } catch (err: any) {
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-24">
      ...
      <div className="flex items-center justify-between sticky top-0 z-40 bg-gray-50/90 dark:bg-[#0A0A0A]/90 backdrop-blur-md py-4 border-b border-gray-200 dark:border-gray-800">
        <div className="flex items-center gap-4">
          <Link href="/admin/ai/case-studies" className="p-2 text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-lg hover:bg-gray-200 dark:hover:bg-gray-800">
