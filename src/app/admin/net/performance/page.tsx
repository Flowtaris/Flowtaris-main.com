...
export default function PerformanceConfigPage() {
  ...
  const handleStringChange = (key: string, value: string | number) => {
    setData((prev: any) => ({
      ...prev,
  ...
  const handleMetricChange = (index: number, key: string, value: string | number) => {
    setData((prev: any) => {
      const newMetrics = [...prev.performance.metrics];
      newMetrics[index] = { ...newMetrics[index], [key]: value };
      return {
        ...prev,
        performance: { ...prev.performance, metrics: newMetrics }
      };
    });
  };
  ...
  const addMetric = () => {
    setData((prev: any) => ({
      ...
      performance: {
        ...prev.performance,
        metrics: [...prev.performance.metrics, { label: "New Metric", value: 0, unit: "unit", desc: "Description" }]
      }
    }));
  };
    ...
    <div className="pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-white flex items-center gap-3">
          ...
          Deterministic Performance Config
        </h1>
        <p className="text-gray-400 mt-2">Manage the live telemetry section, metrics arrays, and success rate highlights.</p>
      </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input type="text" className="w-full bg-[#0a1128] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors font-mono" 
              value={data.performance.tagline} onChange={(e) => handleStringChange('tagline', e.target.value)} />
          </div>
          <div>
            ...
            <input type="text" className="w-full bg-[#0a1128] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors" 
              value={data.performance.title} onChange={(e) => handleStringChange('title', e.target.value)} />
          </div>
          <div className="md:col-span-2">
            ...
            <textarea rows={3} className="w-full bg-[#0a1128] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors" 
              value={data.performance.description} onChange={(e) => handleStringChange('description', e.target.value)} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#151c2f] border border-white/5 rounded-2xl overflow-hidden mb-8 shadow-xl">
        <div className="px-6 py-4 border-b border-white/5 flex items-center gap-2">
          ...
          <h2 className="font-semibold text-white">Highlight Metric (Payload Success Rate)</h2>
        </div>
        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Label</label>
            <input type="text" className="w-full bg-[#0a1128] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors" 
              value={data.performance.payloadSuccessRateLabel} onChange={(e) => handleStringChange('payloadSuccessRateLabel', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Sub-Label</label>
            <input type="text" className="w-full bg-[#0a1128] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors" 
              value={data.performance.payloadSuccessRateDesc} onChange={(e) => handleStringChange('payloadSuccessRateDesc', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Percentage Value</label>
            <input type="number" step="0.01" className="w-full bg-[#0a1128] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors font-mono" 
              value={data.performance.payloadSuccessRate} onChange={(e) => handleStringChange('payloadSuccessRate', parseFloat(e.target.value))} />
          </div>
        </div>
      </div>

      {/* Metrics Array section */}
      <div className="bg-[#151c2f] border border-white/5 rounded-2xl overflow-hidden mb-8 shadow-xl">
        <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-purple-400"></span>
            <h2 className="font-semibold text-white">Dynamic Metrics Array</h2>
          </div>
          <button 
            onClick={addMetric}
          ...
          >
            <span>+</span> Add New Metric
          </button>
        </div>
        <div className="p-6 space-y-6">
          {data.performance.metrics.map((metric: any, idx: number) => (
            <div key={idx} className="bg-[#1a233a] p-5 rounded-xl border border-white/5 relative group">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pr-12">
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Label</label>
                  <input type="text" className="w-full bg-[#0a1128] border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors font-heading" 
                    value={metric.label} onChange={(e) => handleMetricChange(idx, 'label', e.target.value)} />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Value (Number)</label>
                  <input type="number" step="any" className="w-full bg-[#0a1128] border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors font-mono" 
                    value={metric.value} onChange={(e) => handleMetricChange(idx, 'value', parseFloat(e.target.value) || 0)} />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Unit</label>
                  <input type="text" className="w-full bg-[#0a1128] border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors" 
                    value={metric.unit} onChange={(e) => handleMetricChange(idx, 'unit', e.target.value)} />
                </div>
                <div className="md:col-span-3">
                  <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Description</label>
                  <input type="text" className="w-full bg-[#0a1128] border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-colors" 
                    value={metric.desc} onChange={(e) => handleMetricChange(idx, 'desc', e.target.value)} />
                </div>
              </div>
            </div>
          ))}
          {data.performance.metrics.length === 0 && (
            <div className="text-center py-8 text-gray-500 text-sm">No metrics configured. Click "Add New Metric" to create one.</div>
          )}
        </div>
      </div>
